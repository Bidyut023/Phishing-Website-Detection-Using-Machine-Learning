...
def extract_risk_flags(url: str) -> list[str]:
    ...
    trusted_bank_in_variant = hostname.endswith(".bank.in") and any(
        hint in hostname for hint in TRUSTED_BANK_HINTS
    )
    ...
    if hostname.count(".") >= 3 and not trusted_bank_in_variant:
        flags.append("Has many subdomains, which is common in deceptive URLs.")
    if len(lowered) > 75:
        flags.append("The URL is unusually long.")
    if re.search(r"(login|verify|update|secure|confirm|signin|password)", path_and_query):
...
def has_trusted_hint(hostname: str) -> bool:
    return any(hint in hostname for hint in TRUSTED_HOST_HINTS)


def is_likely_official_bank(hostname: str, path: str, query: str, risk_flags: list[str]) -> bool:
    if path not in ("", "/") or query:
        return False
    if risk_flags:
        return False
    if hostname.endswith(".bank.in"):
        return any(hint in hostname for hint in TRUSTED_BANK_HINTS)
    return any(hint in hostname for hint in TRUSTED_BANK_HINTS)


def has_piracy_hint(url: str, hostname: str, path: str) -> bool:
    lowered = url.lower()
    return any(
        hint in lowered or hint in hostname or hint in path
        for hint in PIRACY_HINTS
    )
...
def build_verdict(url: str, score: float, risk_flags: list[str]) -> tuple[str, str, str]:
    ...
    clean_domain_shape = bool(re.fullmatch(r"[a-z0-9.-]+", base_hostname))
    simple_domain = base_hostname.count(".") <= 2 and "--" not in base_hostname
    flag_count = len(risk_flags)

    if has_piracy_hint(url, base_hostname, path):
        return "Bad", "This website matches risky piracy or suspicious download patterns.", "risk"

    if base_hostname in TRUSTED_DOMAINS and flag_count == 0:
        return "Good", "This website looks safe for normal browsing.", "safe"

    if is_likely_official_bank(base_hostname, path, query, risk_flags):
        return "Good", "This bank website looks safe for normal browsing.", "safe"

    if flag_count >= 2 or score <= -1.35:
        return "Bad", "This website shows strong phishing-like behavior.", "risk"

    if clean_root_domain and flag_count == 0 and clean_domain_shape and simple_domain and has_trusted_hint(base_hostname):
        return "Good", "This website looks safe for normal browsing.", "safe"

    if clean_root_domain and flag_count == 0 and clean_domain_shape and simple_domain and score >= -0.8:
        return "Good", "This website looks safe for normal browsing.", "safe"
